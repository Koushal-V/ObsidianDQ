...
def format_3tier_response(state: Dict[str, Any], duration_ms: int) -> Dict[str, Any]:
    ...
    root_cause_analysis = {
        ...
        "root_cause_confidence_score": state.get("root_cause_confidence_score", 0.0),
        ...
        "plan_risk_level": state.get("plan_risk_level", "MEDIUM"),
        "planning_llm_used": state.get("planning_llm_used", False),
        "investigation_llm_used": state.get("investigation_llm_used", False),
        "investigation_summary": state.get("investigation_summary", ""),
    }

    llm_agents = {
        "investigation": bool(state.get("investigation_llm_used", False)),
        "root_cause": bool(state.get("root_cause_llm_used", False)),
        "planning": bool(state.get("planning_llm_used", False)),
    ...
    return {
        ...
        "verification": {
            ...
            "post_remediation_dq": state.get("post_remediation_dq", {}),
            "cleaned_file": state.get("cleaned_file"),
            "recovery_required": state.get("verification_recovery_required", False),
        },
